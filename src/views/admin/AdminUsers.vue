        ...
        <div class="container">
            <AdminHeader name="Пользователи">
                <div @click="isOpen = !isOpen" class="relative flex items-center space-x-2 cursor-pointer select-none">
                    <div class="font-sf_pro font-normal text-sm text-white">{{ currentFilter }}</div>
                    <svg :class="[isOpen ? 'rotate-180' : '']" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div v-if="isOpen" class="absolute top-8 right-0 bg-m_black-100 rounded-lg">
                    ...
                    <tbody>
                        <tr v-for="user in users" :key="user.id"
                            ...
                            user.is_teacher == true ? 'bg-m_yellow-300 text-m_yellow-100' : user.is_staff ? 'bg-green-600 text-green-100' : 'bg-m_blue-200 text-m_blue-300']">
                                    {{ user.is_teacher == true ? 'Наставник' : user.is_staff ? 'Администратор' :
                            'Пользователь' }}
                                </span>
                            </td>
                        </tr>
